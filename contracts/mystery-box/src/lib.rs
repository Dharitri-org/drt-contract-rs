    ...
    + events::EventsModule
{
    ...
    fn init(&self, mystery_box_token_id: TokenIdentifier) {
        require!(
            mystery_box_token_id.is_valid_dct_identifier(),
            "Invalid token ID"
        );
    ...
    fn setup_mystery_box(
        ...
        winning_rates_list: MultiValueEncoded<
            MultiValue6<RewardType, MoaxOrDctTokenIdentifier, BigUint, ManagedBuffer, u64, u64>,
        >,
    ) {
    ...
    fn create_mystery_box(&self, amount: BigUint) -> DctTokenPayment<Self::Api> {
        self.require_caller_is_admin();
        ...
        let caller = self.blockchain().get_caller();
        self.send()
            .direct_non_zero_dct_payment(&caller, &output_payment);
    ...
    fn create_and_send_mystery_box(&self, address: &ManagedAddress) {
        ...
        self.send()
            .direct_non_zero_dct_payment(address, &new_mystery_box_payment);
    }
}
