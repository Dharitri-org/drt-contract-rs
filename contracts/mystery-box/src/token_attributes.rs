...
pub trait TokenAttributesModule: config::ConfigModule {
    fn create_new_tokens<T: TopEncode + NestedEncode>(
        ...
        if existing_nonce != 0 {
            self.send()
                .dct_local_mint(&mystery_box_token_id, existing_nonce, &amount);

            return DctTokenPayment::new(mystery_box_token_id, existing_nonce, amount);
        }
        ...
        self.send()
            .dct_local_mint(&mystery_box_token_id, new_nonce, &amount);

        DctTokenPayment::new(mystery_box_token_id, new_nonce, amount)
    }
